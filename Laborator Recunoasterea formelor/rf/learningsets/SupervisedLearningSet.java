...
public class SupervisedLearningSet  extends UnsupervisedLearningSet {
    ...
    public double[] calculateWeightsValues(){
        ...
        Map<Pattern,Double> patternMap = StatisticsUtils.getPatternsMapFromInitialSet(X);
        f=StatisticsUtils.getPatternWeigths(patternMap);
        this.X = StatisticsUtils.getPatterns(patternMap);
    ...
    private void fillSupervisedFieldsValues(double[][] X, int[] iClass, String[] classNames) {
        ...
        for(int i=0; i<n; i++)
        {
            if(X[i].length!=p)
            {
                throw new RuntimeException("Not all the patterns have the same number of features");
            }
        }
